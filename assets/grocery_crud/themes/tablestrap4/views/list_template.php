<?php
	$this->set_css($this->default_theme_path.'/tablestrap4/css/dataTables.bootstrap4.css');
	...
	$this->set_css($this->default_theme_path.'/tablestrap4/extras/TableTools/media/css/TableTools.css');
	...
	$this->set_js_lib($this->default_theme_path.'/tablestrap4/js/jquery.dataTables.min.js');
...
<div class="grocerycrud-container panel panel-default">
	...
	<div class="dataTablesContainer">
		<div class="panel-body mt-1 ml-3">
			<?php if(!$unset_add){?>
			...
			<?php }?>
			<?php if(isset($actions['add_modal'])){?>
				<?php $add=$actions['add_modal'];?>
				<button type="button" class="mt-1 ml-3 add_button btn btn-success hidden-xs" 
				
			<?php if($add->modal){?>
				data-toggle="modal" data-target='#<?=$add->modalTarget?>' 
			<?php }?>
			<?php if($add->js!=''){?>
				onclick="<?=$add->js?>"
			<?php }?>
				 aria-pressed="false" autocomplete="off"><i class="fa fa-plus"></i> <?=$add->label?></button>
			<?php }?>
			...
			<div class="float-right">
				<?php if(!$unset_export){?>
					<a role="button" class="add_button btn btn-light" href="<?php echo $export_url?>">
						<i class="fas fa-file-download"></i>
						<?php echo $this->l('list_export');?>
					</a>
				<?php }?>
				<?php if(!$unset_export){?>
					<a role="button" class="add_button btn btn-light" href="<?php echo $print_url?>">
