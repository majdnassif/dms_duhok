            ...
            <div class="modal-body p-3 text-right" id="tree-section">
                <form id="AddNode" onsubmit="CallAddNode(); return false;" method="post" <?php if($this->UserModel->language()=="ENGLISH"){?>dir="ltr"<?php }?> >
                        <div class="row <?php if($this->UserModel->language()=="ENGLISH"){?>text-left<?php }?>" >
                           ...
                           <div class="form-group col-md-4" >
                                <label for="code"><?=$this->Dictionary->GetKeyword("Branch");?></label>
                                <select name="branch_id" class="form-control">
                                    <option value="-1"><?=$this->Dictionary->GetKeyword("Select Branch");?></option>
                ...
                <form id="EditNode" onsubmit="CallEditNodeSubmit(); return false;" method="post"  <?php if($this->UserModel->language()=="ENGLISH"){?>dir="ltr"<?php }?>>
                        <div class="row">
                            ...
                            <div class="form-group col-md-4" >
                                <label for="code"><?=$this->Dictionary->GetKeyword("Branch");?></label>
                                <select name="branch_id"  id="EditNodeBranch" class="form-control">
                                    <option value="-1"><?=$this->Dictionary->GetKeyword("Select Branch");?></option>
                                    <?php foreach($branches as $branch):?>
                                        <option value="<?=$branch['id']?>"><?=$branch['name']?></option>
                                    <?php endforeach;?>
...
<script>
    ...
    function CallEditNode(node_id,parent_id){
        ...
        node_branch=$('span[data-id="'+node_id+'"]').data('branch');
        $("#EditNodeName").val(node_name);
        //$("#EditNodeCode").val(node_code);

        // give the node branch to the select element EditNodeBranch
        $("#EditNodeBranch").val(node_branch);
    ...
    function CallEditNodeSubmit(){
        ...
        $.post("<?=base_url("Tree/AjaxEditNode/".$tree_type.'/')?>"+NodeID,{name:$("#EditNodeName").val(),code:$("#EditNodeCode").val(), branch_id: $("#EditNodeBranch").val()},function(response){
            response=JSON.parse(response);
            if(response.status=='True'){
                $("#EditNodeName").val("");
                $("#EditNodeBranch").val("-1");
                $("#EditNodeCode").val("");
                $("#EditNodeID").val("");
