            ...
            if(is_dir($controller)) {
                ...
                foreach(glob(APPPATH . 'controllers/'.$dirname.'/*') as $subdircontroller) {
                    ...
                    foreach($aMethods as $method) {
                        if($method != '__construct' && $method != 'get_instance' && $method != 'index' && $method != 'Login' && $method != 'Logout' && $method != $subdircontrollername && strpos( $method, '_' ) !== 0 && strpos( $method, 'Ajax' ) !== 0) {
                            $aUserMethods[] = $method;
                        }
                    }
                ...
                if(is_array($aMethods)){
                    foreach($aMethods as $method) {
                        if($method != '__construct' && $method != 'get_instance' && $method != 'index' && $method != 'Login' && $method != 'Logout' && $method != $controllername && strpos( $method, '_' ) !== 0 && strpos( $method, 'Ajax' ) !== 0) {
                            $aUserMethods[] = $method;
                        }
                    }
                }
