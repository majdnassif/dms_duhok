...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->add_url				= $this->getAddUrl().$this->extra_param;
		...
		$data->read_url				= $this->getReadUrl().$this->extra_param;
		$data->ajax_list_url		= $this->getAjaxListUrl();
		$data->ajax_list_info_url	= $this->getAjaxListInfoUrl();
		$data->export_url			= $this->getExportToExcelUrl().$this->extra_param;
		$data->print_url			= $this->getPrintUrl().$this->extra_param;
		...
		foreach($data->list as $num_row => $row)
		{
			...
			$data->list[$num_row]->edit_url = $data->edit_url.'/'.$row->{$data->primary_key}.$this->extra_param;
			$data->list[$num_row]->delete_url = $data->delete_url.'/'.$row->{$data->primary_key}.$this->extra_param;;
			$data->list[$num_row]->read_url = $data->read_url.'/'.$row->{$data->primary_key}.$this->extra_param;;
            $data->list[$num_row]->clone_url = $data->clone_url.'/'.$row->{$data->primary_key}.$this->extra_param;;
		}
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class=' form-control  select2' data-placeholder='".$select_title."'>";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$ajax_or_not_class = $using_ajax ? 'select2' : 'select2';

		$this->_inline_js("var ajax_relation_url = '".$this->getAjaxRelationUrl()."';\n");

		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='form-control $ajax_or_not_class ' data-placeholder='$select_title'>";
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		{
			$css_class = $has_priority_field || $is_ie_7 ? 'form-control multiselect': 'form-control chosen-multiple-select';
			...
			$input = "<select id='field-{$field_info_type->name}' name='{$field_info_type->name}[]' multiple='multiple' size='15' class='$css_class' data-placeholder='$select_title'  >";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	{
		return $this->state_url(''.$this->extra_param,true);
	}
	...
	public function set_extra_param_url($value='')
	{
	...
	protected function getEditUrl($primary_key = null)
	{
		if($primary_key === null)
			...
			return $this->state_url('edit/'.$primary_key.'/'.$this->extra_param);
	}
	...
	protected function getReadUrl($primary_key = null)
	{
		if($primary_key === null)
			...
			return $this->state_url('read/'.$primary_key.$this->extra_param);
	}

	protected function getUpdateUrl($state_info)
	{
		return $this->state_url('update/'.$state_info->primary_key.$this->extra_param);
	}
	...
	protected function getDeleteUrl($state_info = null)
	{
		if (empty($state_info)) {
			...
			return $this->state_url('delete/'.$state_info->primary_key.$this->extra_param);
        }
	}

    protected function getDeleteMultipleUrl()
    {
        return $this->state_url('delete_multiple').$this->extra_param;
    }

	protected function getListSuccessUrl($primary_key = null)
	{
		if(empty($primary_key))
			return $this->state_url('success'.$this->extra_param,true);
		else
			return $this->state_url('success/'.$primary_key.$this->extra_param,true);
	}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	 ...
	 * @param $url_callback
	 */
	public function add_action( $label, $image_url = '', $link_url = '', $css_class = '', $url_callback = null,$btn_css_class='')
	{
	...
	public function add_modal( $label, $link_url='',$modal=false,$modalTarget,$js='')
	{
		
		$this->actions['add_modal']  = (object)array(
			'label' 		=> $label,
			'link_url'		=> $link_url,
			'modal'=>$modal,
